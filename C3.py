...
ADMIN_CHAT_ID = 1587278794 # Замените на настоящий chat_id администратора
...
def add_order_number_column():
    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute("PRAGMA table_info(orders)")  # Получаем информацию о таблице
    columns = cursor.fetchall()
    
    # Проверяем, существует ли столбец "order_number"
    if not any(column[1] == "order_number" for column in columns):
...
def get_user_chat_id_by_order_number(order_number):
    conn = create_connection()
    cursor = conn.cursor()
...
@bot.message_handler(func=lambda message: message.text == "Узнать чат ID клиента")
...
def process_order_number(message):
    ...
    user_chat_id = get_user_chat_id_by_order_number(order_number)
    
    if user_chat_id:
        bot.send_message(message.chat.id, f"Чат ID клиента: `{user_chat_id}`", parse_mode="Markdown")
    else:
...
def freeze_dish(message):
    if message.chat.id != ADMIN_CHAT_ID:
        bot.send_message(message.chat.id, "⚠️У вас нет разрешения на использование этой команды.")
        return
...
def add_to_cart_handler(call):
    ...
    if len(data) < 5:
        bot.send_message(call.message.chat.id, "⚠️Ошибка при добавлении блюда в корзину. Неверный формат данных.")
        return
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, "⚠️Ошибка при расчете цены блюда. Пожалуйста, попробуйте снова.")
        return
    ...
    if dish_status and dish_status[0] == 1:  # Dish is frozen
        ...
        try:
            ...
            bot.send_message(call.message.chat.id, "⚠️При добавлении блюда в корзину произошла ошибка. Пожалуйста, повторите попытку позже.")
...
def main_menu(chat_id):
    ...
    b9 = types.KeyboardButton("🔎Поиск 🍕блюда")
...
def handle_contact(message):
    ...
    if message.contact:
        ...
        bot.send_message(message.chat.id, "⚠️Не удалось получить ваш контакт. Пожалуйста, попробуйте снова.")
...
def admin(message):
    ...
    itembtn9 = types.KeyboardButton("Узнать чат ID клиента")
    back_button = types.KeyboardButton("Назад")  # Кнопка назад
    markup.add(itembtn1, itembtn2, itembtn3, itembtn4, itembtn5, itembtn6, itembtn7, itembtn8, itembtn9, back_button)
...
def search_dishes_by_name(query):
    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM menu WHERE name LIKE ?', ('%' + query + '%',))
    results = cursor.fetchall()
    conn.close()
    return results

@bot.message_handler(func=lambda message: message.text == "🔎Поиск 🍕блюда")
def search_dish(message):
    bot.send_message(message.chat.id, "Введите название 🍕блюда(с большим буквам) или его часть для 🔎поиска:")
    bot.register_next_step_handler(message, process_search)

def process_search(message):
    query = message.text.strip()
    if not query:
        bot.send_message(message.chat.id, "Пожалуйста, введите название 🍕блюда для 🔎поиска.")
        return

    # Поиск по введенному запросу
    dishes = search_dishes_by_name(query)

    if not dishes:
        bot.send_message(message.chat.id, "Ничего не найдено по запросу.")
        return

    # Отправляем результаты поиска
    markup = types.InlineKeyboardMarkup()
    for dish in dishes:
        dish_name, price = dish[1], dish[2]  # Предположим, что dish[1] - название и dish[2] - цена
        markup.add(types.InlineKeyboardButton(f"{dish_name} - {price} руб.", callback_data=f"add_to_cart_{dish_name}_{price}"))
...
def add_to_cart_handler(call):
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, "⚠️Ошибка при обработке цены блюда. Пожалуйста, попробуйте снова.")
        return
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, "⚠️Произошла ошибка при добавлении блюда в корзину. Попробуйте позже.")
        print(f"⚠️Ошибка при добавлении в корзину: {e}")
...
# Функция для отправки сообщения курьеру с номером заказа и кнопкой "Завершить"
def send_order_to_courier(user_id, payment_method):
    ...
    phone = phone_row[0] if phone_row else "Не указан"
    ...
    # Отправляем сообщение курьеру с номером заказа и кнопкой "Завершить"
    markup = types.InlineKeyboardMarkup()
    ...
    bot.send_message(user_id, f"🥘 Ваш номер заказа: `{order_number}`. Бот ищет свободного доставщика. Подождите минуту....", parse_mode="Markdown")
...
def complete_order(call):
    ...
    # Сообщаем курьеру, что заказ завершен
    bot.send_message(call.message.chat.id, f"Вы ✅ завершили заказ `{order_number}`.", parse_mode="Markdown")

    # Отправляем сообщение пользователю
    bot.send_message(user_id, f"Ваш заказ `{order_number}` был ✅успешно доставлен! Спасибо, что выбрали нас!", parse_mode="Markdown")
...
def accept_order(call):
    ...
    bot.send_message(call.message.chat.id, f"Вы ✅приняли заказ `{order_number}`.", parse_mode="Markdown")

    # Добавляем кнопку "Завершить"
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("✅ Завершить", callback_data=f"complete_{user_id}_{order_number}"))

    # Отправляем сообщение с кнопкой "Завершить"
    bot.send_message(call.message.chat.id, f"Завершите заказ `{order_number}`", reply_markup=markup, parse_mode="Markdown")

    # Отправляем сообщение пользователю
    bot.send_message(user_id, f"Доставщик ✅принял ваш заказ `{order_number}`. Ожидайте доставку...", parse_mode="Markdown")
...
def reject_order(call):
    ...
    bot.send_message(call.message.chat.id, f"Вы отклонили заказ `{order_number}`.", parse_mode="Markdown")

    # Отправляем сообщение пользователю
    bot.send_message(user_id, f"Доставщик отклонил ваш заказ `{order_number}`. Попробуйте позже.", parse_mode="Markdown")
...
def send_message_to_couriers(message_text):
    ...
    for courier_chat_id in courier_chat_ids:
        try:
            ...
            print(f"⚠️Ошибка при отправке сообщения курьеру с chat_id {courier_chat_id}: {e}")


# Список chat_id курьеров (это должны быть реальные chat_id курьеров)
COURIER_CHAT_IDS = [1587278794, 5720193588]  # Замените на реальные chat_id курьеров
...
courier_chat_ids = [1587278794, 7211194096]
...
def message_user(message):
    ...
    if len(command_parts) < 2:
        bot.send_message(message.chat.id, "⚠️Введите chat_id пользователя и сообщение через пробел.")
        return
    ...
    try:
        ...
        bot.send_message(message.chat.id, "⚠️Ошибка: chat_id пользователя должен быть числом.")
        return
